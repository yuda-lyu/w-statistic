...
import arrAverageWithLogNormCI from './arrAverageWithLogNormCI.mjs'


/**
 * 計算物件陣列內指定鍵值時有效數字取對數(log)後之平均值和標準差，並基於司徒頓t累加分布之信賴水準(p)以及指定左右單尾條件下，計算樣本平均值再取其指數(exp)值。此時左尾代表樣本平均值可小於等於母數平均值，而右尾代表樣本平均值可大於等於母數平均值
 *
 * Unit Test: {@link https://github.com/yuda-lyu/w-statistic/blob/master/test/ltdtAverageWithLogNormCI.test.js Github}
 ...
 *
 */
async function ltdtAverageWithLogNormCI(ltdt, key, p, mode = 'two-tailed') {
    ...
    //arrAverageWithLogNormCI
    let r = await arrAverageWithLogNormCI(rs, p, mode)

    return r
}


export default ltdtAverageWithLogNormCI
