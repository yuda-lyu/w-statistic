...
describe(`arrAverageWithNormCI`, function() {
    ...
    it(`should return [ 39.026745114674725, 27.33689124896163 ] when input [6, 47, 49, 15, 42, 41, 7, 39, 43, 40, 36], 0.25`, async () => {
        let r = await arrAverageWithNormCI([6, 47, 49, 15, 42, 41, 7, 39, 43, 40, 36], 0.25)
        let rr = [39.026745114674725, 27.33689124896163]
        assert.strict.deepStrictEqual(r, rr)
    })

    it(`should return [ 36.53111791210903, 29.832518451527324 ] when input [6, 47, 49, 15, 42, 41, 7, 39, 43, 40, 36], 0.5`, async () => {
        let r = await arrAverageWithNormCI([6, 47, 49, 15, 42, 41, 7, 39, 43, 40, 36], 0.5)
        let rr = [36.53111791210903, 29.832518451527324]
        assert.strict.deepStrictEqual(r, rr)
    })

    it(`should return [ 34.74951166569836, 31.614124697938003 ] when input [6, 47, 49, 15, 42, 41, 7, 39, 43, 40, 36], 0.75`, async () => {
        let r = await arrAverageWithNormCI([6, 47, 49, 15, 42, 41, 7, 39, 43, 40, 36], 0.75)
        let rr = [34.74951166569836, 31.614124697938003]
        assert.strict.deepStrictEqual(r, rr)
    })

    it(`should return [ 2.5700668400779687, 0.42993315992203107 ] when input ['abc', '-2.5', -2.5, '-1', -1, '-0.1', -0.1, '0', 0, '0.1', 0.1, '1', 1, '2.5', 2.5, 22.5, 'xyz'], 0.5`, async () => {
    ...
    it(`should return [ 5.011682286477021, 1.5883177135229787 ] when input ['abc', '0', 0, '0.1', 0.1, '1', 1, '2.5', 2.5, 22.5, 'xyz'], 0.5`, async () => {
